...
const AdminAllSeoList = () => {
  ...
  const fetchNewsAndMedia = () => {
    ...
    fetch(
      `${server_url}/seo/getAllSeo?size=${size}&page=${page}&filter=${input}`
    )
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`${server_url}/seo/deleteSeo/${id}`, {
          method: "DELETE",
        }).then((res) => {
          if (res.status === 200) {
            setRefresh(!refresh);
  ...
  const tableHeader = ["Serial No", "Page", "Image", "Title", "Action"];
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="flex items-center justify-center gap-3">
                        <Link
                          to={`/adminDashboard/update-seo/${item._id}`}
