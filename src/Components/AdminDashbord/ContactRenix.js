...
const ContactRenix = () => {
  const [contactRenix, setContactRenix] = useState([]);
  ...
  const fetchContactRenix = () => {
    ...
    fetch(
      `${server_url}/contactRenix/getContactRenixes?size=${size}&page=${page}&filter=${input}`
    )
      .then((response) => response.json())
      .then((data) => {
        setContactRenix(data.data);
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`${server_url}/contactRenix/deleteContactRenix/${id}`, {
          method: "DELETE",
        }).then((res) => {
          if (res.status === 200) {
            setRefresh(!refresh);
        ...
        <div className="relative overflow-x-auto">
          <table className="w-full text-sm text-left text-gray-500">
            ...
            <tbody>
              ...
              ) : contactRenix?.length === 0 ? (
                <tr>
                  <td colSpan={tableHeader.length} className="py-4 text-center">
                    No Contact Renix Found!
                  </td>
                </tr>
              ) : (
                contactRenix.map((item, index) => (
                  <tr
                    key={index}
