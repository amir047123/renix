...
function ContactRenix() {
  ...
  const [filtecontactRenixs, setFiltecontactRenixs] = useState([]); // State for filtered contactRenixes
  ...
  useEffect(() => {
    ...
    if (!searchTerm) {
      setFiltecontactRenixs(contactRenixes);
    } else {
      handleSearch();
    }
  }, [searchTerm, contactRenixes]);
    ...
    <div className=" m-5">
      ...
      <div className=" mb-3">
        <span className=" text-gray-700">
          {" "}
          Showing {filtecontactRenixs.length} Results
        </span>
      </div>
      ...
      >
        <tbody>
          ...
          {/* Map through the filtered contactRenixes instead of all contactRenixes */}
          {filtecontactRenixs.map((requestMedicine, index) => (
            <tr key={requestMedicine._id} className="shadow">
              ...
              <td className="h-16 px-6 text-sm transition duration-300 border-slate-200 stroke-slate-500 text-slate-500">
                {requestMedicine?.subject}
              </td>
              <td className="h-16 px-6 text-sm transition duration-300 border-slate-200 stroke-slate-500 text-slate-500">
                {requestMedicine?.message}
              </td>
