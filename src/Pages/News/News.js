...
const News = () => {
  ...
  const fetchNewsAndMedia = () => {
    setLoading(true);
    fetch("http://localhost:5000/api/v1/newsAndMedia/specific?fieldName=newsCategory&fieldValue=News")
            ...
            {newsAndMedia.map((news, index) => (
              <a href={news?.youtubeLink} target="_blank" className="px-2 py-2 shadow-md" key={index}>
                ...
                <p
                  ...
                  dangerouslySetInnerHTML={{ __html: news?.newsDescription.slice(0, 250) }}
                ></p>
              </a>
            ))}
          </div>
